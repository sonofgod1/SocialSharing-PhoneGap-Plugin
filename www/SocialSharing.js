...
SocialSharing.prototype.shareViaTwitterWithPasteMessageHint = function (message, file, url, successCallback, errorCallback,pasteMessageHint,imageBack) {
  var fileArray = this._asArray(file);
  var ecb = this._getErrorCallback(errorCallback, "shareViaTwitterWithPasteMessageHint");

  pasteMessageHint = pasteMessageHint || "If you like you can paste a message from your clipboard";
  if (fileArray.length > 1) {
    ecb("shareViaTwitter supports max one file");
  } else {
    cordova.exec(successCallback, ecb, "SocialSharing", "shareViaTwitterWithPasteMessageHint", [message, null, fileArray, url, pasteMessageHint,imageBack]);
  }
};
...
SocialSharing.prototype.shareViaFacebookWithPasteMessageHint = function (message, fileOrFileArray, url, successCallback, errorCallback,pasteMessageHint,imageBack) {
  pasteMessageHint = pasteMessageHint || "If you like you can paste a message from your clipboard";
  cordova.exec(successCallback, this._getErrorCallback(errorCallback, "shareViaFacebookWithPasteMessageHint"), "SocialSharing", "shareViaFacebookWithPasteMessageHint", [message, null, this._asArray(fileOrFileArray), url, pasteMessageHint,imageBack]);
};
