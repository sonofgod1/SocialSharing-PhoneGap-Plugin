...
- (void)shareViaTwitterWithPasteMessageHint:(CDVInvokedUrlCommand*)command {
    // If Twitter app is installed a message is not prefilled.
    ...
    if (message != (id)[NSNull null]) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 1000 * NSEC_PER_MSEC), dispatch_get_main_queue(), ^{
            BOOL fbAppInstalled = [[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"twitter://"]]; // requires whitelisting on iOS9
            if (fbAppInstalled) {
